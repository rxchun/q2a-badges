	...
	class qa_badge_page {
		...
		function process_request($request)
		{
			...
			foreach($badges as $slug => $info) {
				...
				$types = $type['slug'];
				$typen = $type['name'];
				$qa_content['custom'.++$c]='<table class="badge-entry entry-'.$types.'"><tr id="badge-anchor-'.$slug.'" class="badge-entry-badge"><td><span class="badge-'.$types.'" title="'.$typen.'">'.$name.'</span></td> <td><span class="badge-entry-desc">'.$desc.'</span></td>'.(isset($count[$slug])?' <td><span title="'.$count[$slug]['count'].' '.qa_lang('badges/awarded').'" class="badge-count-link" onclick="jQuery(\'#badge-users-'.$slug.'\').toggleClass(\'q2a-show-badge-source\')">x'.$count[$slug]['count'].'</span></td>':'<td><span class="badge-count">0</span></td>').'</tr>';
